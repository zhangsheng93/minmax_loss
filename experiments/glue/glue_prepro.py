...
def main(args):
    ...
    multi_test_dig_path = os.path.join(root, 'MNLI/diagnostic.tsv')
    ...
    multinli_ax_test_data = load_ax(multi_test_dig_path, is_train=False)
    logger.info('Loaded {} MNLI ax test samples'.format(len(multinli_ax_test_data)))
    ...
    multinli_ax_test_fout = os.path.join(canonical_data_root, 'mnli_ax_test.tsv')
    dump_rows(multinli_ax_test_data, multinli_ax_test_fout, DataFormat.PremiseAndOneHypothesis)
    logger.info('done with ax')
