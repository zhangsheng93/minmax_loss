...
class ACLSampler(BatchSampler):
    def __init__(self, datasets, batch_size, controller, shuffle=True):
        self._datasets = datasets
        # print("batch+++++size =======", batch_size)
        self._batch_size = batch_size
        self.lengths = []
        train_data_list = []
        self.n_task = len(datasets)
        ...
        self.controller.set_lengths([len(d) for d in train_data_list])
        print("controller lengths", self.controller.lengths)
        self.controller.initalization()
    ...
    def get_task_iter(self, idx):
        if self.shuffle:
            random.shuffle(self._train_data_list[idx])
        return iter(self._train_data_list[idx])

    def __iter__(self):
        self.controller.initalization()
        self.all_iters = [self.get_task_iter(idx) for idx in range(self.n_task)]
        return self
    ...
    def __next__(self):
        ...
        if local_task_idx is not None:
            ...
            try:
                ...
                self.all_iters[local_task_idx] = self.get_task_iter(local_task_idx)
                batch = next(self.all_iters[local_task_idx])
...
class Collater:
    ...
    def _prepare_model_input(self, batch, data_type):
        ...
        if self.__if_pair__(data_type):
            ...
            hypothesis_masks = torch.BoolTensor(batch_size, hypothesis_len).fill_(1)
